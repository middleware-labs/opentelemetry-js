...
function refCandidatePaths(ref: string): string[] {
  return [
    ref,
    `refs/${ref}`,
    `refs/tags/${ref}`,
...
/**
 * Resolves a ref (e.g. `HEAD`, a branch, or a tag) to a commit SHA by
 * walking `.git/<ref>` / `.git/packed-refs`, without shelling out to `git`
 * or depending on a git implementation library.
 */
function resolveRefInternal(gitdir: string, ref: string): string {
...
function hasOddNumberOfQuotes(text: string): boolean {
...
/** Reads `remote.<name>.url` for every `[remote "<name>"]` section in a git config file. */
function listRemoteUrls(configText: string): Map<string, string> {
...
export function resolveOriginUrl(repoDir: string): string | undefined {
